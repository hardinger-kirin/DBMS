...
December 5:
    Created select count(*) from function
    Created select avg() from function
    Created select max() from function
...
def selectCount(cwd, command):
    command = helper.removePhrase(command, "select  count(*) from ")
    table_name = command.split(' ', 1)[0]
    ...
    #ensures table exists
    if not os.path.exists(newpath) :
        print(helper.bcolors.RED + "!Failed to select count(*) from " + table_name + " because it does not exist." + helper.bcolors.END)
        return
    
    #opens table file for reading and counts number of lines excluding the first line
    #displays number of lines
    with open(table_name, 'r') as f:
        lines = f.readlines()
        print(str(len(lines) - 1))

def selectAvg(cwd, command):
    command = helper.removePhrase(command, "select  avg(")
    column = command.split(' ', 1)[0]
    column = column.replace(")", '')
    command = helper.removePhrase(command, column + ") from ")
    ...
    newpath = cwd + "/" + table_name
    if not os.path.exists(newpath):
        print(helper.bcolors.RED + "!Failed to select avg(" + column + ") from " + table_name + " because it does not exist." + helper.bcolors.END)
        return
    
    #opens table file for reading, finds columns and index of selected column
    with open(table_name, 'r') as f:
        columns = f.readline().strip().split(" | ")
    ...
    found = False
    for i in range(len(columns)):
        if columns[i] == column and not found:
            found = True
            index = i
            break
        elif found:
    ...
    #calculates average of column values and displays it
    total = 0
    num_rows = 0
    with open(table_name, 'r') as f:
        lines = f.readlines()
        ...
        for i in range(1, len(lines)):
            ...
            num_rows += 1
    print(str(total / num_rows))

def selectMax(cwd, command):
    command = helper.removePhrase(command, "select  max(")
    column = command.split(' ', 1)[0]
    column = column.replace(')', '')
    command = helper.removePhrase(command, column + ") from ")
    ...
    newpath = cwd + "/" + table_name
    if not os.path.exists(newpath):
        print(helper.bcolors.RED + "!Failed to select max(" + column + ") from " + table_name + " because it does not exist." + helper.bcolors.END)
        return
    ...
    #calculates max of column values and displays it
    max = 0
    with open(table_name, 'r') as f:
        lines = f.readlines()
        #skips first line
        for i in range(1, len(lines)):
            line = lines[i].strip()
            if(line.split(" | ")[index].find(".") != -1):
                if(float(line.split(" | ")[index]) > max):
                    max = float(line.split(" | ")[index])
            else:
                if(int(line.split(" | ")[index]) > max):
                    max = int(line.split(" | ")[index])
    print(str(max))
